    ...
    jobs: {
      ...
      items: {
        ...
        properties: {
          ...
          url: { type: "string", description: "Link to the job or application, if present" },
...
export async function extractJobs(text: string, opts: { sourceKind: SourceKind; source: string; postedAt?: string }): Promise<RawJob[]> {
  const body = clean(text).slice(0, 60_000);
  if (!body) return [];
  ...
  return jobs.filter((j) => j.title).map((j) => ({ ...j, sourceKind: opts.sourceKind, source: opts.source, postedAt }));
}
