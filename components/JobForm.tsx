...
export default function JobForm({
  ...
  return (
    ...
    >
      <Field label={t("Job posting")} hint={t("paste the full description")}>
        <Textarea
          rows={6}
          value={job.description}
          onChange={(e) => set("description", e.target.value)}
          placeholder={t("Paste the job ad from SEEK, Indeed, a Facebook group or the centre's website")}
        />
      </Field>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Field label={t("Job title")}>
          <Input value={job.title} onChange={(e) => set("title", e.target.value)} placeholder={t("Diploma Qualified Educator")} required />
        </Field>
        <Field label={t("Centre / employer")}>
          <Input value={job.centre} onChange={(e) => set("centre", e.target.value)} placeholder={t("Little Sprouts Early Learning Centre")} />
        </Field>
        <Field label={t("Location")}>
          <Input value={job.location} onChange={(e) => set("location", e.target.value)} placeholder={t("Parramatta NSW")} />
        </Field>
        <Field label={t("Pay")}>
          <Input value={job.salary} onChange={(e) => set("salary", e.target.value)} placeholder={t("$32–$36/hour")} />
        </Field>
        ...
        <Field label={t("State")}>
          <Select value={job.state} onChange={(v) => set("state", v)} options={[{ value: "", label: t("Choose…") }, ...AU_STATES.map((s) => ({ value: s.id, label: t(s.name) }))]} />
        </Field>
        <div className="grid grid-cols-2 gap-3">
          <Field label={t("Job type")}>
            <Select value={job.roleType} onChange={(v) => set("roleType", v)} options={[{ value: "", label: t("Choose…") }, ...ROLE_TYPES.map((r) => ({ value: r, label: t(r) }))]} />
          </Field>
          <Field label={t("Employment")}>
            <Select value={job.employmentType} onChange={(v) => set("employmentType", v)} options={[{ value: "", label: t("Choose…") }, ...EMPLOYMENT_TYPES.map((r) => ({ value: r, label: t(r) }))]} />
          </Field>
        </div>
      </div>
      ...
      <Field label={t("Philosophy")} hint={t("in the centre's own words")}>
        <Textarea rows={2} value={job.centrePhilosophy} onChange={(e) => set("centrePhilosophy", e.target.value)} placeholder={t("e.g. Every child is capable and curious; families are our partners")} />
      </Field>
      <Field label={t("Programs")}>
        <Textarea rows={2} value={job.centrePrograms} onChange={(e) => set("centrePrograms", e.target.value)} placeholder={t("e.g. Nursery, toddler and funded kindy rooms; weekly bush kinder")} />
      </Field>
      ...
      <Field label={t("Notes")}>
        <Textarea rows={3} value={job.notes} onChange={(e) => set("notes", e.target.value)} placeholder={t("Contacts, follow-ups, questions to ask…")} />
      </Field>
      ...
      <div className="flex justify-end gap-2 pt-2">
        ...
        <Button type="submit">{t("Save job")}</Button>
      </div>
    </form>
  );
}
